...
pub fn query_expand(input: &DeriveInput) -> TokenStream {
    ...
    let list_fn_name: TokenStream = format!(
        "list_{}",
    ...
    quote! {
        ...
        impl #query_name {
            async fn #list_fn_name(
                ...
                paging: Option<Pagination>,
                filter: Option<#filter_name>,
                sorting: Option<Vec<#sort_name>>,
            ) -> async_graphql::Result<async_graphql::connection::Connection<Cursor, #name>> {
                let db = ctx.data::<sea_orm::DatabaseConnection>()?;
                let authorizer=#authorizer_constructor;
                let authorize_condition=Authorizer::authorize(&authorizer,ctx).await?;
                ...
                let start_index = paging.as_ref().map_or(Ok(0), |v| {
                    v.after
                        .as_ref()
                        .map_or(Ok(0), |v| <Cursor as async_graphql::connection::CursorType>::decode_cursor(v).map(|v| v.offset))
                })? + 1;
                connection
                    .edges
                    .extend(
